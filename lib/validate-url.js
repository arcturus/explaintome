...
function normalizeIpv6(ip) {
  ...
  const dottedMatch = withoutZone.match(/(\d{1,3}(?:\.\d{1,3}){3})$/);
  if (!dottedMatch) return withoutZone;

  const parts = dottedMatch[1].split('.').map(Number);
  if (parts.some((n) => !Number.isInteger(n) || n < 0 || n > 255)) return withoutZone;

  const high = (parts[0] * 256 + parts[1]).toString(16);
  const low = (parts[2] * 256 + parts[3]).toString(16);
  return `${withoutZone.slice(0, dottedMatch.index)}${high}:${low}`;
}
